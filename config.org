...
(defun +toggle-inline-images ()
  (interactive)
  (cond
   ((derived-mode-p 'org-mode) (org-toggle-inline-images))
   ((derived-mode-p 'markdown-mode) (markdown-toggle-inline-images))))

(defun +open-at-point ()
  (interactive)
  (cond
   ((derived-mode-p 'org-mode) (org-open-at-point))
   ((derived-mode-p 'markdown-mode) (markdown-follow-thing-at-point nil))))
#+end_src
...
(+def-theme-function base16-hopscotch     "Base16 Hopscotch"     'base16-hopscotch-theme)
(+def-theme-function base16-zos           "Base16 Zos")
...
(defun +switch-window-then-delete-window ()
  (interactive)
  (switch-window--then
   "Delete window: "
   #'delete-window
...
(defun +switch-window-then-split-below-switch (arg)
  (interactive "P")
  (switch-window--then
   "Below-split window: "
   #'+split-window-below-switch
   #'+split-window-below-switch arg 1))

(defun +switch-window-then-split-right-switch (arg)
  (interactive "P")
  (switch-window--then
   "Right-split window: "
   #'+split-window-right-switch
   #'+split-window-right-switch arg 1))
#+end_src
...
** TODO NOT_WORKING Option to show file name in consult-async searches
#+begin_src emacs-lisp
...
** Option to save custom-variables per line
#+begin_src emacs-lisp
(defcustom +custom-save-variables-per-line nil
  "Whether to save custom-set-variables with one token per line"
  :type 'boolean
  :group 'havner)

(defun +custom-save-variables-maybe-per-line (fun &rest args)
  (when +custom-save-variables-per-line
    (set-fill-column 5))
  (apply fun args))
(advice-add #'custom-save-variables :around #'+custom-save-variables-maybe-per-line)
#+end_src
...
*** LOAD
#+begin_src emacs-lisp
(defvar +system-color-depth 'default)
...
(cond (window-system
      ...
      ((equal (getenv "TERM") "xterm-24bit")
       (setq +system-color-depth '24bit))
      ((equal (getenv "TERM") "xterm-ghostty")
       (setq +system-color-depth '24bit))
...
(pcase +system-color-depth
  ...
  ;; ('24bit    (+doom-zenburn))
  ('24bit    (+base16-hopscotch))
  ('256color (+zenburn)))

;; get rid of background on transparent terminals
(if (and (not window-system)
         (equal (getenv "TERM") "foot"))
...
(define-key query-replace-map (kbd "ESC") 'quit)
...
(setq +custom-save-variables-per-line t)
...
(pcase window-system
  ...
  ('pgtk (set-face-attribute 'default nil :font "Ubuntu Mono-12"))
...
(pcase system-type
  ((or 'gnu/linux 'darwin)
   (let ((orig-tfd temporary-file-directory))
     (setq temporary-file-directory
           (concat orig-tfd
                   (if (string-suffix-p "/" orig-tfd) "" "/")
                   (user-login-name)))
...
;; company-indent-for-tab-command is broken, don't remap
(setq company-mode-map (make-sparse-keymap))
...
(setq doom-modeline-modal-icon (if window-system t nil))
...
(setq writeroom-added-width-left 0)
...
(setq projectile-frecency-file "~/.emacs-projectile-frecency.eld")
...
(with-eval-after-load 'leader-key
...
#+begin_src emacs-lisp
;;; === OBSOLETE ===
;;; Orderless now has its own orderless-affix-dispatch with orderless-affix-dispatch-alist
;; (setq orderless-style-dispatchers '(+vertico-orderless-dispatch))
;;; Don't add prefixes to defaults, limit the defaults, use affixes
;; (with-eval-after-load 'orderless
;;   (add-to-list 'orderless-matching-styles 'orderless-prefixes t))

;;; === MAIN EMACS ===
;;; basic as a fallback, works like literal-prefix
(setq completion-styles '(orderless basic))
;;; nullify the defaults, they would override the above
(setq completion-category-defaults nil)
;;; partial-completion as a fallback for files, allows for /u/l/b/, works like prefixes
(setq completion-category-overrides '((file (styles orderless partial-completion))))

;;; === MAIN ORDERLESS ===
;;; just regexp, remove literal from defaults
(setq orderless-matching-styles '(orderless-regexp))
;;; add other styles not present either in orderless-matching-styles or orderless-affix-dispatch-alist
(with-eval-after-load 'orderless
  (add-to-list 'orderless-affix-dispatch-alist `(?@ . ,#'orderless-prefixes) t)
  (add-to-list 'orderless-affix-dispatch-alist `(?- . ,#'orderless-without-literal) t))
#+end_src

| style                     | letter | description                          | example                    |
|---------------------------+--------+--------------------------------------+----------------------------|
| orderless-annotation      | &      | search further columns only          | MODIFIER                   |
| orderless-not             | !      | results without the match            | MODIFIER                   |
| orderless-regexp          |        | regexp                               | l.s..pac -> list-packages  |
| orderless-literal         | =      | exact match within                   | st-pack -> list-packages   |
| orderless-without-literal | -      | everything that doesn't have         | something -> list-packages |
| orderless-literal-prefix  | ^      | starts with literal                  | list-p -> list-packages    |
| orderless-flex            | ~      | loose letters in order               | ltckg -> list-packages     |
| orderless-initialism      | ,      | words starting with letters in order | lp -> list-packages        |
| orderless-prefixes        | @      | beginning of words split with '-'    | li-pa -> list-packages     |
| char-fold-to-regexp       | %      | ignore diactrics in searched elem    | test -> tęst               |
...
(with-eval-after-load 'lsp-clangd
  (add-to-list 'lsp-clients-clangd-args "--log=verbose" t))
...
(add-to-list 'auto-mode-alist '("/bashrc\\.[a-z]+\\'" . shell-script-mode))
(add-to-list 'auto-mode-alist '("/bash_profile\\.[a-z]+\\'" . shell-script-mode))
...
** RON
#+begin_src emacs-lisp
(autoload 'ron-mode "ron-mode" "Major mode for Rusty Object Notation" t)
...
(with-eval-after-load 'company
  (define-key company-active-map (kbd "<escape>") #'company-abort))  ;; OR: +company-abort-and-god-mode-enable
...
(global-set-key (kbd "C-h K") #'describe-keymap)                                    ;; command Info-goto-emacs-key-command-node
(global-set-key (kbd "C-h M") #'describe-minor-mode)                                ;; nil
(global-set-key (kbd "C-h t") #'describe-theme)                                     ;; help-with-tutorial

(global-set-key (kbd "C-x b") #'consult-buffer)                                     ;; switch-to-buffer
(global-set-key (kbd "C-x i") #'ibuffer)                                            ;; insert-file
...
(global-set-key (kbd "C-x 0") #'+switch-window-then-delete-window)                  ;; delete-window
(global-set-key (kbd "C-x 1") #'switch-window-then-maximize)                        ;; delete-other-windows
(global-set-key (kbd "C-x 2") #'+switch-window-then-split-below-switch)             ;; split-window-below
(global-set-key (kbd "C-x 3") #'+switch-window-then-split-right-switch)             ;; split-window-right
#+end_src
...
(with-eval-after-load 'lsp-mode
  ...
  (define-key lsp-mode-map (kbd "C-,") #'consult-lsp-symbols))
...
(with-eval-after-load 'projectile
  ...
  (define-key projectile-command-map (kbd "4 f") #'consult-projectile-find-file-other-window)
  (define-key projectile-command-map (kbd "5 f") #'consult-projectile-find-file-other-frame)
  ...
  (define-key projectile-command-map (kbd "e") #'consult-projectile-recentf)
  (define-key projectile-command-map (kbd "b") #'consult-projectile-switch-to-buffer)
  (define-key projectile-command-map (kbd "4 b") #'consult-projectile-switch-to-buffer-other-window)
  (define-key projectile-command-map (kbd "5 b") #'consult-projectile-switch-to-buffer-other-frame)
...
(with-eval-after-load 'lsp-mode
  ...
  (define-key lsp-command-map (kbd "a") #'lsp-ui-sideline-apply-code-actions)
...
(leader-key-define-key "RET" #'+open-at-point)
...
(leader-key-define-key "0" #'+switch-window-then-delete-window "delete-window")
(leader-key-define-key "1" #'switch-window-then-maximize "maximize")
(leader-key-define-key "2" #'+switch-window-then-split-below-switch "split-below")
(leader-key-define-key "3" #'+switch-window-then-split-right-switch "split-right")
(leader-key-define-key "=" #'balance-windows)

(leader-key-describe-key "x" "emacs")
(leader-key-define-key "x SPC" #'rectangle-mark-mode)
(leader-key-define-key "x ;" #'eval-expression)
(leader-key-define-key "x c" #'save-buffers-kill-terminal)
...
(leader-key-define-key "t i" #'+toggle-inline-images)
...
(leader-key-define-key "w 0" #'+switch-window-then-delete-window "delete-window")
(leader-key-define-key "w 1" #'switch-window-then-maximize "maximize")
(leader-key-define-key "w 2" #'+switch-window-then-split-below-switch "split-below")
(leader-key-define-key "w 3" #'+switch-window-then-split-right-switch "split-right")
(leader-key-define-key "w =" #'balance-windows)
(leader-key-define-key "w k" #'+switch-window-then-kill-buffer-and-window "kill-buffer-and-window")
(leader-key-define-key "w m" #'switch-window-then-maximize "maximize")
...
(leader-key-define-key "b M" #'consult-minor-mode-menu)
...
(leader-key-define-key "h B" #'embark-bindings-in-keymap)
...
(leader-key-define-key "h t" #'describe-theme)
...
| C-y |                      |                                         |         |
|-----+----------------------+-----------------------------------------+---------|
| 0   | delete-window        | +switch-window-then-delete-window       |         |
| 1   | delete-other-windows | switch-window-then-maximize             |         |
| 2   | split-window-below   | +switch-window-then-split-below-switch  |         |
| 3   | split-window-right   | +switch-window-then-split-right-switch  |         |
| 7   |                      |                                         |         |
| 9   |                      |                                         |         |
|-----+----------------------+-----------------------------------------+---------|
