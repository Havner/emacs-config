...
#+BEGIN_SRC emacs-lisp
  ...
  (defun havner/pre-ediff ()
    ...
    (remove-hook 'switch-window-finish-hook 'balance-windows))
  (defun havner/post-ediff ()
    (set-window-configuration havner/last-winconfig)
    (add-hook 'switch-window-finish-hook 'balance-windows))
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun havner/split-window-below-balance-and-switch ()
  ...
  (defun havner/split-window-right-balance-and-switch ()
  ...
  ;;;;; OLD: extensions to switch-window (e.g. kill-window)

  ;; (defun havner/switch-window-then-kill-buffer ()
  ;;     "switch-window--then kill-current-buffer"
  ;;   (interactive)
  ;;   (switch-window--then
  ;;    "Kill buffer in window: "
  ;;    #'havner/kill-current-buffer
  ;;    #'havner/kill-current-buffer t 1))

  ;; (defun havner/switch-window-then-kill-buffer-and-window ()
...
#+BEGIN_SRC emacs-lisp
  ...
  (cl-case havner/colors
    ('24bit (havner/zenburn-load))
    ('256color (havner/solarized-dark-load)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq havner/delete-trailing-whitespace t)
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;; not required per se, but I want avy-pop-mark
  (require 'avy)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq havner/cc-base 'lsp)              ; lsp, rtags
  (setq havner/cc-flycheck 'lsp)          ; lsp, rtags, irony
  (setq havner/cc-company 'lsp)           ; lsp, rtags, irony
  (setq havner/python-base 'lsp)          ; lsp
  (setq havner/python-flycheck 'lsp)      ; lsp, pycheck
  (setq havner/python-company 'lsp)       ; lsp, jedi
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list-global 'flycheck-disabled-checkers 'lsp-ui t)

  ;;; runtime helpers
  (defun havner/lsp-find-project-root ()
    (if (bound-and-true-p projectile-mode)
        (let ((projectile-require-project-root nil))
          (projectile-project-root))
      default-directory))
  (defun havner/lsp-ui-flycheck-setup ()
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list-global 'flycheck-disabled-checkers 'irony t)

  ;;; runtime
  (defun havner/cc-irony-setup ()
    (if (or (eq havner/cc-company 'irony)
            (eq havner/cc-flycheck 'irony))
        (irony-mode t)))

  (with-eval-after-load 'cc-vars
    (add-hook 'c-mode-common-hook #'havner/cc-irony-setup))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list-global 'flycheck-disabled-checkers 'rtags t)
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq havner/use-cquery-package t)
  ...
  ;;; runtime
  (defun havner/cc-cquery-setup ()
    (when (eq havner/cc-base 'lsp)
      (if havner/use-cquery-package
          (lsp-cquery-enable)
        (lsp-cquery-custom-enable)))
    (when (eq havner/cc-flycheck 'lsp)
      (havner/lsp-ui-flycheck-setup)))

  (with-eval-after-load 'cc-vars
    (add-hook 'c-mode-common-hook #'havner/cc-cquery-setup))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (with-eval-after-load 'python-environment
    (setq python-environment-virtualenv
          (append '("virtualenv-3") (delete "virtualenv" python-environment-virtualenv))))

  ;;; runtime
  (defun havner/python-jedi-setup ()
    (when (eq havner/python-company 'jedi)
      (jedi:setup)))

  (with-eval-after-load 'python
    (add-hook 'python-mode-hook #'havner/python-jedi-setup))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (add-to-list-global 'flycheck-disabled-checkers 'python-pycheckers t)
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  ;;; runtime
  (defun havner/python-pyls-setup ()
    (when (eq havner/python-base 'lsp)
      (lsp-pyls-enable))
    (when (eq havner/python-flycheck 'lsp)
      (havner/lsp-ui-flycheck-setup)))

  (with-eval-after-load 'python
    (add-hook 'python-mode-hook #'havner/python-pyls-setup))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun cc-devel-hook-f ()
    ...
    (with-real-file
     ...
     (flycheck-mode t)
     (cl-case havner/cc-flycheck
       ('irony
        (flycheck-disable-checker 'irony t)
        (flycheck-select-checker 'irony))
       ('rtags
        (flycheck-disable-checker 'rtags t)
        (flycheck-select-checker 'rtags))
       ('lsp
        (flycheck-disable-checker 'lsp-ui t)
...
#+BEGIN_SRC emacs-lisp
  ...
  (defun python-devel-hook-f ()
    ...
    (with-real-file
     ...
     (flycheck-mode t)
     (cl-case havner/python-flycheck
       ('pycheck
        (flycheck-disable-checker 'python-pycheckers t)
...
#+BEGIN_SRC emacs-lisp
  ...
  (global-set-key (kbd "C-x C-k") #'havner/kill-buffer-and-window-and-balance)

  (global-set-key (kbd "C-x 0") #'switch-window-then-delete)                    ;; thr 2+
  (global-set-key (kbd "C-x 1") #'switch-window-then-maximize)                  ;; thr 2+
  (global-set-key (kbd "C-x 2") #'havner/split-window-below-balance-and-switch)
  (global-set-key (kbd "C-x 3") #'havner/split-window-right-balance-and-switch)
  ...
  (global-set-key (kbd "C-c <up>") #'symbol-overlay-jump-prev)
  (global-set-key (kbd "C-c <down>") #'symbol-overlay-jump-next)
  (global-set-key (kbd "C-c <left>") #'symbol-overlay-switch-backward)
  (global-set-key (kbd "C-c <right>") #'symbol-overlay-switch-forward)
...
** Test
#+BEGIN_SRC lisp
  ;; (defvar c-summary-timer nil)

  ;; (defun havner/restart-timer ()
  ;;   (interactive)
  ;;   (havner/stop-timer)
  ;;   (havner/start-timer))

  ;; (defun havner/start-timer ()
  ;;   (interactive)
  ;;   (when (eq c-summary-timer nil)
  ;;       (setq c-summary-timer
  ;;             (run-with-idle-timer
  ;;              3 nil 'rtags-display-summary))))

  ;; (defun havner/stop-timer ()
  ;;   (interactive)
  ;;   (unless (eq c-summary-timer nil)
  ;;     (cancel-timer c-summary-timer)
  ;;     (setq c-summary-timer nil)))

  ;; (add-hook 'post-self-insert-hook #'havner/restart-timer)
  ;; (remove-hook 'post-self-insert-hook 'havner/setup-timer)
#+END_SRC
